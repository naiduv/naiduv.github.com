...
var isNodeWebkit = (typeof window["c2nodewebkit"] !== "undefined") || (window.location.search === "?nw" || /nodewebkit/i.test(navigator.userAgent));
...
function doRefreshPoll()
{
	...
	// Don't add the date in node-webkit mode, since it seems to leak cache files
	if (isNodeWebkit)
...
function onMessage(e)
{
	...
	else if (type === "add-watch")
	{
		...
		updateInspectedInstance();
		localStorage.setItem("__c2_watch", JSON.stringify(watch));
	}
